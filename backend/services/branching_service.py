...
class BranchingService:
    ...
    def __init__(
        ...
        if gemini_service:
            ...
            logger.info("🔴 BranchingService: GeminiService를 직접 생성")
    ...
    async def analyze_branching_potential(
        self, user_message: str, ai_response: str, conversation_history: list[dict[str, Any]] = None
    ) -> dict[str, Any]:
    ...
    def _create_recommendations_from_analysis(
        self, branch_types: dict[str, list[str]], complexity_score: float
    ) -> list[dict[str, Any]]:
        """분석 결과로부터 추천 브랜치 생성 (API 호출 없음)"""
        recommendations = []

        # 복잡도에 따라 최대 브랜치 수 결정
        max_branches = min(int(complexity_score * 5), self.max_branches_per_node)
    ...
    async def _generate_branch_recommendations(
        self, context: str, branch_analysis: dict[str, list[str]], complexity_score: float
    ) -> list[dict[str, Any]]:
        """추천 브랜치 생성"""
        recommendations = []
            ...
            for branch_type in priority_order:
                ...
                for branch_title in branches:
                    ...
                    description = await self._generate_branch_description(
                        branch_title, branch_type, context
                    )
    ...
    async def _generate_branch_description(self, title: str, branch_type: str, context: str) -> str:
        """브랜치 설명 생성"""
        try:
            type_descriptions = {
                "topics": "주제를 깊이 탐구",
    ...
    async def create_smart_branches(
        self, parent_node_id: str, recommendations: list[dict[str, Any]], auto_approve: bool = False
    ) -> list[dict[str, Any]]:
        """스마트 브랜치 생성"""
        created_branches = []
        ...
        try:
            ...
            for recommendation in recommendations:
                ...
                try:
                    ...
                    node_type = recommendation.get(
                        "node_type", node_type_mapping.get(branch_type, "topic")
                    )
                    ...
                    if branch_node:
                        ...
                        if hasattr(branch_node, "id"):
                            ...
                            branch_dict = (
                                branch_node.model_dump()
                                if hasattr(branch_node, "model_dump")
                                else branch_node.dict()
                            )
                        else:
                            node_id = branch_node["id"]
                        ...
                        logger.info(
                            f"[브랜치 생성] WebSocket 이벤트 전송 준비: node_id={node_id}, parent_id={parent_node_id}, session_id={session_id}"
                        )
                        await connection_manager.broadcast(node_event, session_id)
                        logger.info(
                            f"[브랜치 생성] WebSocket 이벤트 전송 완료: {node_event['type']}"
                        )
                        ...
                        logger.info(
                            f"[브랜치 생성] 스마트 브랜치 생성 완료: {recommendation['title']}, node_id={node_id}"
                        )
    ...
    def _build_context(
        self, user_message: str, ai_response: str, history: list[dict[str, Any]] = None
    ) -> str:
        """컨텍스트 구성"""
        context_parts = []
    ...
    def _estimate_depth(self, branch_type: str) -> int:
        """예상 깊이 추정"""
        depth_map = {"topics": 5, "details": 3, "questions": 2, "alternatives": 4, "examples": 2}

        return depth_map.get(branch_type, 3)

    def _generate_reasoning(
        self, complexity_score: float, branch_analysis: dict[str, list[str]]
    ) -> str:
        """분기 추천 이유 생성"""
    ...
    async def check_context_limit(
        self, node_id: str, token_limit: int = 4000
