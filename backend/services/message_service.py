...
from backend.schemas.service_responses import ConversationHistory
...
class MessageService:
    ...
    async def list_messages(
        self, node_id: str | None = None, skip: int = 0, limit: int = 50
    ) -> list[Message]:
        """메시지 목록 조회"""
        try:
            if node_id:
    ...
    async def get_conversation_history(
        self, node_id: str, include_ancestors: bool = True
    ) -> ConversationHistory:
        ...
        try:
            ...
            if node_result:
                ...
                # 참조 노드인 경우 특별 처리
                if is_reference:
                    return await self._get_reference_node_history(
                        node_id, node_data.get("source_node_ids")
                    )
            ...
            if include_ancestors:
                ...
                if node_ids_to_fetch:
                    ...
                    messages_result = await self.db.execute_query(
                        messages_query, {"node_ids": node_ids_to_fetch}
                    )
            ...
            return ConversationHistory(
                messages=messages, total_tokens=int(total_tokens), is_summarized=is_summarized
            )

        except Exception as e:
            logger.error(f"대화 기록 조회 실패: {e}")
            return ConversationHistory(messages=[], total_tokens=0, is_summarized=False)

    async def _get_reference_node_history(
        self, node_id: str, source_node_ids_str: str | None = None
    ) -> ConversationHistory:
        ...
        try:
            ...
            if parent_result:
                ...
                parent_result[0].get("parent_type")

                # 부모 노드의 대화 기록 가져오기 (재귀적으로 처리됨)
                parent_history = await self.get_conversation_history(
                    parent_id, include_ancestors=True
                )
            ...
            RETURN m
            """
            own_messages_result = await self.db.execute_query(
                own_messages_query, {"node_id": node_id}
            )
    ...
    async def _get_node_messages_only(self, node_id: str) -> ConversationHistory:
        ...
        try:
            ...
            return ConversationHistory(
                messages=messages, total_tokens=int(total_tokens), is_summarized=False
            )
    ...
    async def update_message_embedding(self, message_id: str, embedding: list[float]) -> bool:
        """메시지 임베딩 업데이트"""
        try:
            query = """
            MATCH (m:Message {id: $id})
            ...
            result = await self.db.execute_query(query, {"id": message_id, "embedding": embedding})
