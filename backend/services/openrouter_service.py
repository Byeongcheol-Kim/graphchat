...
class OpenRouterService:
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                ...
                for i, msg in enumerate(messages[-3:], 1):  # 최근 3개 메시지만 표시
                    logger.debug(f"  [{msg.get('role', 'unknown')}]: {str(msg.get('content', ''))[:200]}{'...' if len(str(msg.get('content', ''))) > 200 else ''}")
                ...
                if response.status_code == 200:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        ...
                        if 'choices' in data and len(data['choices']) > 0:
                            ...
                            logger.debug(f"응답 내용: {content[:500]}{'...' if len(content) > 500 else ''}")
                        if 'usage' in data:
                            logger.debug(f"토큰 사용량: 입력={data['usage'].get('prompt_tokens', 0)}, 출력={data['usage'].get('completion_tokens', 0)}, 총={data['usage'].get('total_tokens', 0)}")
                        logger.debug(f"{'='*50}\n")
                    ...
                    logger.warning(f"일일 무료 한도 초과: {error_data.get('error', {}).get('message', 'Unknown error')}")
    ...
    async def stream_chat_completion(
        ...
        try:
            if not self.api_key or self.api_key == "your-openrouter-api-key":
                ...
                logger.warning("OpenRouter API 키가 설정되지 않았습니다. 모의 스트리밍을 사용합니다.")
    ...
    def _mock_response(self, messages: List[Dict[str, str]], response_format: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """모의 응답 생성"""
