...
class BranchRecommendationService:
    ...
    async def create_recommendation(
        self, recommendation: BranchRecommendationCreate
    ) -> BranchRecommendation:
        """단일 브랜치 추천 생성"""
        try:
    ...
    async def create_recommendations_batch(
        self, batch: BranchRecommendationBatch
    ...
    async def get_recommendations_for_message(self, message_id: str) -> list[BranchRecommendation]:
    ...
    async def get_recommendations_for_node(
        self, node_id: str, status_filter: RecommendationStatus | None = None
    ) -> list[BranchRecommendation]:
        """특정 노드의 모든 브랜치 추천 조회"""
        try:
            if status_filter:
    ...
    async def update_recommendation(
        self, recommendation_id: str, update: BranchRecommendationUpdate
    ) -> BranchRecommendation:
        """브랜치 추천 상태 업데이트"""
        try:
            update_fields = []
    ...
    async def mark_as_created(
        self, recommendation_id: str, created_branch_id: str
    ) -> BranchRecommendation:
        """브랜치 생성 완료 표시"""
        update = BranchRecommendationUpdate(
            status=RecommendationStatus.CREATED, created_branch_id=created_branch_id
        )
        return await self.update_recommendation(recommendation_id, update)

    async def mark_as_dismissed(self, recommendation_id: str) -> BranchRecommendation:
        """브랜치 추천 무시 표시"""
        update = BranchRecommendationUpdate(
            status=RecommendationStatus.DISMISSED, dismissed_at=datetime.utcnow()
        )
        return await self.update_recommendation(recommendation_id, update)
