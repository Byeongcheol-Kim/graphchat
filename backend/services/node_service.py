...
from backend.schemas.node import NodeCreate, NodeUpdate, Node, NodeTree, SummaryRequest, NodeRelations
from backend.schemas.service_responses import NodeStatistics, SummaryResult, DeleteNodesResult
...
class NodeService:
    ...
    def __init__(self, db: FalkorDBManager, chat_service: Optional['ChatService'] = None, branching_service: Optional['BranchingService'] = None) -> None:
        self.db = db
        ...
        try:
            ...
            if result:
                ...
                if hasattr(node_data_raw, '__dict__'):
                    ...
                    node_dict = dict(node_data_raw) if not isinstance(node_data_raw, dict) else node_data_raw
                
                # metadata가 문자열로 반환된 경우 딕셔너리로 변환
                metadata = {}
                metadata_value = node_dict.get('metadata') if hasattr(node_dict, 'get') else node_dict['metadata']
                ...
                created_node = Node(
                    ...
                    updated_at=datetime.fromisoformat(safe_get(node_dict, "updated_at", safe_get(node_dict, "created_at"))),
        ...
        try:
            ...
            if result:
                ...
                return Node(
                    ...
                    created_at=datetime.fromisoformat(safe_get(node_dict, "created_at")),
                    updated_at=datetime.fromisoformat(safe_get(node_dict, "updated_at", safe_get(node_dict, "created_at"))),
                    token_count=safe_get(node_dict, "token_count", message_count * 100),  # 메시지 기반 토큰 추정
    ...
    async def get_node_with_messages(self, node_id: str) -> Optional[Dict[str, Any]]:
        """메시지를 포함한 노드 조회"""
        try:
    ...
    async def get_node_tree(self, node_id: str) -> Optional[NodeTree]:
        ...
        try:
            ...
            for r in result:
                ...
                if "metadata" in child_data:
                    try:
                        import json
                        metadata = json.loads(child_data["metadata"]) if isinstance(child_data["metadata"], str) else child_data.get("metadata", {})
                    except:
                        metadata = {}
                ...
                children.append(Node(
                    ...
                    updated_at=datetime.fromisoformat(child_data.get("updated_at", child_data["created_at"])),
    ...
    async def list_nodes(
        ...
        try:
            ...
            if parent_id:
                ...
                LIMIT $limit
                """
                params = {"session_id": session_id, "parent_id": parent_id, "skip": skip, "limit": limit}
            else:
                query = """
            ...
            for row in result:
                ...
                node = Node(
                    ...
                    updated_at=datetime.fromisoformat(node_data.get("updated_at", node_data["created_at"])),
                    token_count=node_data.get("token_count", message_count * 100),  # 메시지 기반 토큰 추정
    ...
    async def update_node(
        ...
        try:
            ...
            if update_data.metadata is not None:
                ...
                # metadata를 JSON 문자열로 변환
                import json
                metadata_str = json.dumps(update_data.metadata) if isinstance(update_data.metadata, dict) else update_data.metadata
    ...
    async def delete_nodes(self, node_ids: List[str], include_descendants: bool = False) -> DeleteNodesResult:
        """여러 노드 삭제
        ...
        try:
            if include_descendants:
                ...
                if delete_result and len(delete_result) > 0:
                    ...
                    if deleted_count < len(node_ids):
                        ...
                        result.deleted_node_ids = [nid for nid in node_ids if nid not in remaining_ids]
            ...
            if result.success:
                ...
                result.message = f"{result.deleted_count}개 노드 삭제, {len(result.failed_node_ids)}개 실패"
    ...
    async def create_summary_old(
        ...
        try:
            ...
            for source_id in node_ids:
                ...
                """
                await self.db.execute_write(link_query, {
                    "source_id": source_id,
                    "summary_id": node_id
                })
            ...
            return Node(
                ...
                updated_at=datetime.fromisoformat(summary_node.get("updated_at", summary_node["created_at"])),
    ...
    async def create_reference(
        ...
        try:
            ...
            return Node(
                ...
                updated_at=datetime.fromisoformat(reference_node.get("updated_at", reference_node["created_at"])),
                message_count=0,
    ...
    async def get_node_descendants(self, node_id: str, max_depth: Optional[int] = None) -> List[Node]:
