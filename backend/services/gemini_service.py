...
class GeminiService:
    ...
    async def chat_completion(
        self, messages: list[Message], temperature: float = 0.7, structured: bool = False
    ) -> ChatResponse:
        """
        채팅 완성 생성 - 메시지 응답만 생성
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                ...
                logger.debug(
                    f"응답 내용: {response.text[:500]}{'...' if len(response.text) > 500 else ''}"
                )
                logger.debug(f"{'=' * 50}\n")

            return ChatResponse(content=response.text, finish_reason="stop")
...
단순한 대화나 명확한 답변이 이미 제공된 경우 빈 배열을 반환합니다."""
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"\n[Gemini 브랜칭 분석 응답]\n{'-' * 50}")
                logger.debug(
                    f"원본 응답: {response.text[:500]}{'...' if len(response.text) > 500 else ''}"
                )
    ...
    def _mock_completion(self) -> ChatResponse:
        """모의 채팅 응답 생성"""
        return ChatResponse(content="이것은 Gemini 모의 응답입니다.", finish_reason="stop")
