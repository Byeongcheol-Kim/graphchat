...
from backend.schemas.message import ChatRequest, ChatResponse, MessageCreate, Message
...
from backend.schemas.ai_models import Message as AIMessage, BranchType
...
class ChatService:
    ...
    def __init__(self, 
                 ...
                 gemini_model: str = "gemini-2.0-flash-exp") -> None:
        self.db = db
    ...
    async def process_chat(
        ...
        try:
            ...
            conversation = await self.message_service.get_conversation_history(request.node_id, include_ancestors=True)
            ...
            if request.auto_branch:
                ...
                # 컨텍스트 한계 확인
                is_near_limit, token_count, summary = await self.branching_service.check_context_limit(
                    node_id=request.node_id,
                    token_limit=4000
                )
...
2. summary: 대화 내용의 종합적인 요약"""
    ...
    async def generate_summary_with_instructions(self, contents: List[str], instructions: str) -> SummaryResult:
        """지침 기반 요약 생성 (타이틀 포함)"""
        try:
            # 각 노드의 내용을 명확히 구분
            formatted_contents = []
...
2. summary: 지침에 따른 대화 내용 요약"""
            ...
            if response and response.content:
                try:
                    ...
                    if not title_text:
                        # 지침에서 타이틀 추출
                        title_text = instructions[:20] if len(instructions) <= 20 else instructions[:17] + "..."
    ...
    async def _generate_node_summary_if_needed(self, node_id: str) -> Optional[SummaryResult]:
        """노드가 부모가 될 때 요약 생성"""
        try:
            # 현재 노드의 메시지 가져오기
    ...
    def _prepare_messages(self, history: List[Message]) -> List[AIMessage]:
        ...
        messages = [
            AIMessage(role="system", content="당신은 도움이 되는 AI 어시스턴트입니다.")
        ]
