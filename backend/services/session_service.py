...
from backend.schemas.session import SessionCreate, SessionUpdate, Session, SessionWithNodes
from backend.schemas.node import Node
from backend.schemas.service_responses import (
    SessionWithStatistics, 
    SessionStatistics, 
    PaginatedSessions,
    OperationResult,
    NodeStatistics
...
class SessionService:
    ...
    async def update_session(
        ...
        try:
            set_clauses = []
            params = {"id": session_id, "updated_at": datetime.now(timezone.utc).isoformat()}
    ...
    async def get_session_with_nodes(self, session_id: str) -> Optional[SessionWithNodes]:
        """노드를 포함한 세션 조회"""
        try:
