...
class ConnectionManager:
    ...
    async def broadcast(self, message: Dict, session_id: str, exclude: WebSocket = None):
        ...
        logger.info(f"[WebSocket] 브로드캐스트 시작: session_id={session_id}, message_type={message.get('type')}")
            ...
            if connection != exclude:
                try:
                    ...
                    if connection.client_state.name != "CONNECTED":
                        logger.warning(f"[WebSocket] 연결되지 않은 상태: {connection.client_state.name}")
