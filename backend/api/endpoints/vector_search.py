...
async def search_similar_nodes(
    ...
    """
    try:
        logger.info(f"벡터 검색 요청: query='{request.query[:50]}...', session={request.session_id}")
        
        result = await service.search(request)
        
        logger.info(f"벡터 검색 완료: {result.total_results}개 결과 (소요시간: {result.search_time_ms:.2f}ms)")
