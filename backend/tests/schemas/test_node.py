...
class TestNodeSchemas:
    ...
    def test_node_base_valid(self):
        """유효한 NodeBase 생성 테스트"""
        node = NodeBase(title="테스트 노드", type="user", metadata={"key": "value"})
    ...
    def test_node_create_summary(self):
        """요약 노드 생성 스키마 테스트"""
        node = NodeCreate(
            title="요약", type="summary", is_summary=True, summary_content="요약 내용"
        )
    ...
    def test_node_update(self):
        """NodeUpdate 스키마 테스트"""
        update = NodeUpdate(title="수정된 노드", is_active=False, metadata={"updated": True})
    ...
    def test_node_with_messages(self):
        ...
        now = datetime.now()
        message = Message(
            id="msg-123", node_id="node-123", role="user", content="메시지 내용", timestamp=now
        )
    ...
    def test_node_tree(self):
        ...
        # 트리 구조
        tree = NodeTree(node=root, children=[NodeTree(node=child, children=[])])
