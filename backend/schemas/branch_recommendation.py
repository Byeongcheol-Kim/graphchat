...
class BranchRecommendationUpdate(BaseModel):
    ...
    created_branch_id: Optional[str] = Field(None, description="생성된 브랜치 ID (status가 CREATED일 때)")
    dismissed_at: Optional[datetime] = Field(None, description="무시한 시간 (status가 DISMISSED일 때)")
