...
class ChatResponse(BaseModel):
    ...
    usage: Optional[dict] = Field(default=None, description="토큰 사용량")
    finish_reason: Optional[str] = Field(default=None, description="종료 이유")


class BranchingRequest(BaseModel):
    """브랜칭 분석 요청"""
    messages: List[Message] = Field(description="분석할 대화 메시지")
    temperature: float = Field(default=0.3, ge=0.0, le=1.0, description="분석 온도 (낮을수록 일관성)")
...
class SmartBranchRecommendation(BaseModel):
    ...
    rationale: Optional[str] = Field(default=None, description="추천 이유")
